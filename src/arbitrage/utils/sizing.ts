...
export function scalingMultiplier(edgeBps: number, mode: SizeScalingMode): number {
...
export function computeSizeUsd(params: {
  ...
  const remainingMarket = Math.max(0, params.maxPositionUsd - params.currentMarketExposureUsd);
  const remainingWallet = Math.max(0, params.maxWalletExposureUsd - params.currentWalletExposureUsd);
  const sizeUsd = Math.max(0, Math.min(target, remainingMarket, remainingWallet));
