...
export class ArbRiskManager implements RiskManager {
  ...
  constructor(params: { config: ArbConfig; state: StateStore; logger: Logger; wallet: Wallet }) {
    this.config = params.config;
    this.state = params.state;
    this.logger = params.logger;
    this.wallet = params.wallet;
    this.opportunityCache = new TtlLruSet(OPPORTUNITY_CACHE_SIZE, OPPORTUNITY_TTL_MS);
    this.startupReadyAt = Date.now() + this.config.startupCooldownSeconds * 1000;
  }
  ...
  canExecute(opportunity: Opportunity, now: number): { allowed: boolean; reason?: string } {
    ...
    if (now < this.startupReadyAt) return { allowed: false, reason: 'startup_cooldown' };
    if (fs.existsSync(this.config.killSwitchFile)) return { allowed: false, reason: 'kill_switch' };

    const cooldown = this.state.getMarketCooldown(opportunity.marketId);
    if (cooldown && cooldown > now) return { allowed: false, reason: 'market_cooldown' };
    ...
    if (this.state.getConsecutiveFailures() >= this.config.maxConsecutiveFailures) {
    ...
    if (walletExposure + opportunity.sizeUsd * 2 > this.config.maxWalletExposureUsd) {
  ...
  async onTradeSubmitted(opportunity: Opportunity, now: number): Promise<void> {
    ...
    this.state.setMarketCooldown(opportunity.marketId, now + this.config.marketCooldownSeconds * 1000);
  ...
  async onTradeSuccess(opportunity: Opportunity, now: number): Promise<void> {
    ...
    await this.state.snapshot();
    this.logger.info(`[ARB] Trade success ${opportunity.marketId} size=${opportunity.sizeUsd.toFixed(2)} USD`);
  }

  async onTradeFailure(opportunity: Opportunity, now: number, reason: string): Promise<void> {
    this.state.incrementFailure();
    await this.state.snapshot();
    this.logger.warn(`[ARB] Trade failure ${opportunity.marketId}: ${reason}`);
  }

  async ensureGasBalance(_now: number): Promise<{ ok: boolean; balance: number }> {
    const polBalance = await getPolBalance(this.wallet);
    if (polBalance < this.config.minPolGas) {
      this.logger.warn(`[ARB] POL balance below minimum: ${polBalance.toFixed(4)} < ${this.config.minPolGas}`);
