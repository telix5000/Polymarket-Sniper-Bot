...
const parseGwei = (gwei: number): BigNumber => utils.parseUnits(String(gwei), 'gwei');

/**
 * Estimates EIP-1559 gas fees for Polygon with safe defaults
 * Applies floors + multipliers to ensure RPC acceptance
 */
export const estimateGasFees = async (params: GasEstimateParams): Promise<GasEstimate> => {
  const multiplier = params.multiplier ?? parseFloat(readEnv('POLY_GAS_MULTIPLIER') || '1.2');
  const minPriorityFeeGwei = params.maxPriorityFeeGwei ?? parseFloat(readEnv('POLY_MAX_PRIORITY_FEE_GWEI') || '30');
  const minMaxFeeGwei = params.maxFeeGwei ?? parseFloat(readEnv('POLY_MAX_FEE_GWEI') || '60');
  ...
  try {
    ...
    let maxPriorityFeePerGas = feeData.maxPriorityFeePerGas || parseGwei(minPriorityFeeGwei);
    ...
    // Apply multiplier
    maxPriorityFeePerGas = maxPriorityFeePerGas.mul(Math.floor(multiplier * 100)).div(100);
    ...
    params.logger?.warn(`[Gas] Failed to fetch fee data, using defaults: ${error}`);
    const maxPriorityFeePerGas = parseGwei(minPriorityFeeGwei).mul(Math.floor(multiplier * 100)).div(100);
    const maxFeePerGas = parseGwei(minMaxFeeGwei).mul(Math.floor(multiplier * 100)).div(100);
...
export const retryTxWithBackoff = async <T>(
  ...
  const maxAttempts = params.maxAttempts ?? parseInt(readEnv('APPROVALS_MAX_RETRY_ATTEMPTS') || '3', 10);
