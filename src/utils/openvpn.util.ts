...
const getOpenVpnEnv = (): OpenVpnEnv => {
  const configPath = readEnv('OPENVPN_CONFIG_PATH') || OPENVPN_DEFAULT_CONFIG_PATH;
  return {
...
const writeConfig = async (env: OpenVpnEnv): Promise<void> => {
  ...
  await fs.writeFile(env.configPath, `${env.config.trim()}
`, { mode: 0o600 });
};
...
const writeAuth = async (env: OpenVpnEnv): Promise<boolean> => {
  ...
  await fs.writeFile(env.authPath, `${env.username}
${env.password}
`, { mode: 0o600 });
  return true;
};
...
export async function startOpenvpn(logger: Logger): Promise<boolean> {
  ...
  try {
    ...
    if (!hasAuth && (env.username || env.password)) {
      throw new Error('OPENVPN_USERNAME and OPENVPN_PASSWORD must both be set.');
    }
