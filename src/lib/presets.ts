...
export const PRESETS: Record<Preset, PresetConfig> = {
  conservative: {
    ...
    hedge: { enabled: true, triggerPct: 15, maxUsd: 15, reservePct: 25, allowExceedMax: false, absoluteMaxUsd: 25 },
    scalp: { enabled: true, minProfitPct: 15, minGainCents: 8, minProfitUsd: 2.0, lowPriceThreshold: 0 },
  ...
  balanced: {
    ...
    hedge: { enabled: true, triggerPct: 20, maxUsd: 25, reservePct: 20, allowExceedMax: false, absoluteMaxUsd: 50 },
    scalp: { enabled: true, minProfitPct: 10, minGainCents: 5, minProfitUsd: 1.0, lowPriceThreshold: 0 },
  ...
  aggressive: {
    ...
    hedge: { enabled: true, triggerPct: 25, maxUsd: 50, reservePct: 15, allowExceedMax: true, absoluteMaxUsd: 100 },
    scalp: { enabled: true, minProfitPct: 5, minGainCents: 3, minProfitUsd: 0.5, lowPriceThreshold: 0 },
...
export function loadPreset(): { name: Preset; config: PresetConfig } {
  const name = (process.env.STRATEGY_PRESET ?? process.env.PRESET ?? "balanced") as Preset;
  if (!PRESETS[name]) {
