...
test("HEDGING settings from aggressive preset are loaded correctly", () => {
  ...
  const config = loadStrategyConfig();
  // Aggressive preset has these hedging settings (see presets.ts)
...
test("Hedging config respects absoluteMaxUsd over maxHedgeUsd when allowExceedMax is true", () => {
  resetEnv();
  Object.assign(process.env, baseEnv, {
    STRATEGY_PRESET: "aggressive",
    MAX_POSITION_USD: "5",
  ...
  // This is verified by the hedging.ts logic, not just config loading
});
