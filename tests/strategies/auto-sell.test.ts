...
describe("AutoSell Stale Expiry Hold Logic", () => {
  ...
  test("position expiring within hold window should be held (not sold)", () => {
    ...
    assert.strictEqual(
      result,
      true,
      "Position expiring in 24h should be held (within 48h window)",
    );
  });
  ...
  test("position expiring AFTER hold window should be sold", () => {
    ...
    assert.strictEqual(
      result,
      false,
      "Position expiring in 120h should NOT be held (outside 48h window)",
    );
  });
  ...
  test("position without marketEndTime should be eligible for sale", () => {
    ...
    assert.strictEqual(
      result,
      false,
      "Position without marketEndTime should NOT be held",
    );
  });
  ...
  test("expiry hold disabled when staleExpiryHoldHours is 0", () => {
    ...
    assert.strictEqual(
      result,
      false,
      "Position should NOT be held when expiryHoldHours is 0",
    );
  });
  ...
  test("position at exact boundary of hold window", () => {
    ...
    assert.strictEqual(
      result,
      true,
      "Position at exact boundary should be held (<=, not <)",
    );
  });
});
