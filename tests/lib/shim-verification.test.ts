...
describe("applyEthersV6Shim", () => {
  ...
  test("shim does not modify wallet if _signTypedData already exists", () => {
    ...
    const typedWallet = wallet as Wallet & { _signTypedData?: () => Promise<string> };
    typedWallet._signTypedData = customFn;
    
    // Apply the shim
    const shimmedWallet = applyEthersV6Shim(typedWallet);
    const resultWallet = shimmedWallet as Wallet & { _signTypedData?: () => Promise<string> };
  ...
  test("shimmed _signTypedData returns same as signTypedData", async () => {
    ...
    const signTypedDataResult = await wallet.signTypedData(domain, types, value);
    const _signTypedDataResult = await typedWallet._signTypedData!(domain, types, value);
