...
describe("V2 Minimum Tradeable Price Filter", () => {
  ...
  describe("preOrderCheck SELL price validation", () => {
    test("SELL at price exactly equal to ABSOLUTE_MIN_TRADEABLE_PRICE should be rejected", () => {
      ...
      assert.strictEqual(result.ok, false, "SELL at minimum price should be rejected");
      assert.ok(result.reason !== undefined, "Rejection should include a reason");
      assert.ok(
    ...
    test("SELL at price below ABSOLUTE_MIN_TRADEABLE_PRICE should be rejected", () => {
      ...
      assert.strictEqual(result.ok, false, "SELL below minimum price should be rejected");
      assert.ok(result.reason !== undefined, "Rejection should include a reason");
      assert.ok(
    ...
    test("SELL at price of 0 should be rejected", () => {
      ...
      assert.strictEqual(result.ok, false, "SELL at zero price should be rejected");
      assert.ok(result.reason !== undefined, "Rejection should include a reason");
    });
    ...
    test("SELL at price just above ABSOLUTE_MIN_TRADEABLE_PRICE should be allowed", () => {
      ...
      assert.strictEqual(result.ok, true, "SELL just above minimum should be allowed");
      assert.strictEqual(result.reason, undefined, "No rejection reason for valid price");
    });

    test("SELL at normal price (e.g., 50¢) should be allowed", () => {
      const result = checkSellPrice(0.50);
      
      assert.strictEqual(result.ok, true, "SELL at normal price should be allowed");
    });

    test("SELL at high price (e.g., 99¢) should be allowed", () => {
      const result = checkSellPrice(0.99);
      
      assert.strictEqual(result.ok, true, "SELL at high price should be allowed");
    });
  });
  ...
  describe("Position loop filter for untradeable prices", () => {
    ...
    test("Position with normal price should be processed", () => {
      const shouldProcess = shouldProcessPosition(0.50);
  ...
  describe("Edge cases", () => {
    test("Very small positive price just below minimum should be rejected", () => {
      ...
      assert.strictEqual(result.ok, false, "Price just below minimum should be rejected");
    });

    test("Very small positive price just above minimum should be allowed", () => {
      const result = checkSellPrice(0.0011); // Just above 0.001
      
      assert.strictEqual(result.ok, true, "Price just above minimum should be allowed");
    });
