...
describe("marketId behavior in order execution", () => {
  ...
  it("should verify cache TTL constants", () => {
    ...
    assert.strictEqual(SUCCESS_TTL_MS, 3600000, "Success cache should be 1 hour");
    ...
    assert.ok(SUCCESS_TTL_MS > ERROR_TTL_MS * 10, "Success TTL should be much longer than error TTL");
  });
  ...
  it("should verify cleanup removes correct percentage", () => {
    ...
    assert.strictEqual(removeCount, 2000, "Should remove 2000 entries (20% of 10000)");
    
    // After cleanup: 10000 - 2000 = 8000, then +1 new entry = 8001
    const sizeAfterCleanup = maxSize - removeCount + 1;
    assert.ok(sizeAfterCleanup < maxSize, "After cleanup should have room for new entries");
    assert.strictEqual(sizeAfterCleanup, 8001, "Should have 8001 entries after cleanup + new entry");
  });
  ...
  it("should verify cache eviction prevents unbounded growth", () => {
    ...
    assert.strictEqual(cleanupTarget, 8000, "Cleanup should target 8000 entries");
  });
});
...
describe("marketId cache cleanup edge cases", () => {
  ...
  it("should verify cleanup preserves in-flight requests", () => {
    ...
    assert.ok(true, "In-flight request preservation during cleanup is documented");
  });
});
