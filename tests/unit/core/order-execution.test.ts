...
describe("Order Execution Module", () => {
  describe("Shared execution path verification", () => {
    ...
    it("computeExecutionLimitPrice is exported and usable by both paths", () => {
      ...
      assert.ok(buyResult.limitPrice >= 0.60, "BUY limit must be >= bestAsk");
      ...
      assert.ok(sellResult.limitPrice <= 0.50, "SELL limit must be <= bestBid");
    });

    it("placeOrderWithFallback validates inputs consistently", async () => {
      const { validateBeforePlacement } = require("../../../src/lib/order-execution");
      ...
      assert.ok(dustBook.reason?.includes("UNHEALTHY_BOOK"), `Expected UNHEALTHY_BOOK, got ${dustBook.reason}`);
    });
  });
  ...
  describe("Strategy bounds enforcement", () => {
    ...
    it("rejects BUY when bestAsk > STRATEGY_MAX", () => {
      const result = computeExecutionLimitPrice({
        bestBid: 0.68,
        bestAsk: 0.70, // Above default STRATEGY_MAX (0.65)
        side: "BUY",
        slippageFrac: 0.06,
      });
    ...
    it("rejects SELL when bestBid < STRATEGY_MIN", () => {
      const result = computeExecutionLimitPrice({
        bestBid: 0.30, // Below default STRATEGY_MIN (0.35)
        bestAsk: 0.32,
        side: "SELL",
        slippageFrac: 0.06,
      });
  ...
  describe("Book health validation", () => {
    const { isBookHealthyForExecution } = require("../../../src/lib/price-safety");
    ...
    it("rejects crossed book (bid > ask)", () => {
      const result = isBookHealthyForExecution(0.60, 0.55);
      assert.strictEqual(result.healthy, false);
  ...
  describe("Rejection classification", () => {
    ...
    it("classifies common rejection reasons", () => {
      assert.strictEqual(classifyRejectionReason("Invalid price increment"), "PRICE_INCREMENT");
      assert.strictEqual(classifyRejectionReason("Insufficient balance"), "INSUFFICIENT_BALANCE");
      assert.strictEqual(classifyRejectionReason("Post only order would trade"), "POST_ONLY_WOULD_TRADE");
      assert.strictEqual(classifyRejectionReason("Order size too small"), "MIN_SIZE");
      assert.strictEqual(classifyRejectionReason("Stale nonce"), "STALE_ORDERBOOK");
      assert.strictEqual(classifyRejectionReason("Market closed"), "MARKET_CLOSED");
    });

    it("returns UNKNOWN for unrecognized errors", () => {
      assert.strictEqual(classifyRejectionReason("Some random error"), "UNKNOWN");
    });
  });
  ...
  describe("Acceptance criteria", () => {
    ...
    it("with bid=0.59 ask=0.60 slippage=6%, BUY limit is >=0.60 and <=strategyMax", () => {
      ...
      assert.strictEqual(result.success, true);
      assert.strictEqual(result.basePrice, 0.60, "basePriceUsed must be bestAsk");
      assert.ok(result.limitPrice >= 0.60, `limit ${result.limitPrice} must be >= bestAsk 0.60`);
      assert.ok(result.limitPrice <= 0.65, `limit ${result.limitPrice} must be <= strategyMax 0.65`);
    ...
    it("with bid=0.59 ask=0.60 slippage=6%, SELL limit is <=0.59 and >=strategyMin", () => {
      ...
      assert.strictEqual(result.success, true);
      assert.strictEqual(result.basePrice, 0.59, "basePriceUsed must be bestBid");
      assert.ok(result.limitPrice <= 0.59, `limit ${result.limitPrice} must be <= bestBid 0.59`);
      assert.ok(result.limitPrice >= 0.35, `limit ${result.limitPrice} must be >= strategyMin 0.35`);
    ...
    it("ORDER_PRICE_DEBUG log includes correct basePriceUsed", () => {
      ...
      assert.strictEqual(buyResult.basePrice, 0.60);
