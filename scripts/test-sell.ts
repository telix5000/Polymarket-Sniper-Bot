...
async function testSellOrder(
  ...
  try {
    ...
    if (bestBid < minPrice) {
      logger.warn(
        `Best bid ${(bestBid * 100).toFixed(0)}¢ is below minimum ${(minPrice * 100).toFixed(0)}¢`,
      );
      logger.warn(
        `This would result in >${SLIPPAGE_TOLERANCE * 100}% loss from entry price`,
      );
...
async function main() {
  ...
  logger.info(`━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`);
  logger.info(
    `Mode: ${executeMode ? "⚡ EXECUTE (will actually sell)" : "🔍 DRY RUN (simulation only)"}`,
  );
  logger.info(`━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`);
  ...
  for (let i = 0; i < positions.length; i++) {
    ...
    logger.info(
      `   Value: $${p.value.toFixed(2)} | P&L: ${p.pnlPct >= 0 ? "+" : ""}${p.pnlPct.toFixed(1)}%`,
    );
  }
  ...
  logger.info(
    `Selected: ${testPosition.outcome} (highest profit: ${testPosition.pnlPct >= 0 ? "+" : ""}${testPosition.pnlPct.toFixed(1)}%)`,
  );
  ...
  if (executeMode) {
    ...
    await new Promise((resolve) =>
      setTimeout(resolve, EXECUTE_WARNING_DELAY_MS),
    );
  }
  ...
  if (result.success) {
    if (executeMode) {
      ...
      logger.info(
        `   New balance: $${newBalance.toFixed(2)} (was $${balance.toFixed(2)})`,
      );
    ...
    if (result.reason === "NO_BIDS") {
      ...
      logger.info(
        `   This is expected behavior - bot won't sell at bad prices.`,
      );
    } else {
      logger.error(
