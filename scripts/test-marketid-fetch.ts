...
async function testMarketIdFetch() {
  ...
  console.log(
    `📋 Test 1: Fetching market info for token ${testTokenId.slice(0, 16)}...`,
  );
  ...
  try {
    ...
    if (marketInfo) {
      ...
      console.log(
        `   question: ${marketInfo.question?.slice(0, 50) || "N/A"}...`,
      );
      console.log(`   tokens: ${marketInfo.tokens.length}`);

      for (const token of marketInfo.tokens) {
        console.log(
          `     - ${token.outcomeLabel} (idx=${token.outcomeIndex}): ${token.tokenId.slice(0, 16)}...`,
        );
      }
      ...
      console.log(
        `✅ Success! (${elapsedTime2}ms - ${elapsedTime2 < elapsedTime ? "faster, using cache!" : "similar speed"})`,
      );
      console.log(
        `   marketId matches: ${marketInfo2?.marketId === marketInfo.marketId ? "✓" : "✗"}`,
      );
    } else {
